...
For $\mathbf{x} = (x_1,\dots,x_n)$, where $x_i$ is known, with $f_0$ and $f_1$ being probability density functions. We assume that 
\begin{equation}
f_{X_i}(X_i = x_i) = \pi_0 f_0(x_i) + \pi_1 f_1(x_i)
(\#eq:e1)
\end{equation}
...
We can classify $x_i$ to be 1st group if $T_{0,i}=P(z_i=0|\mathbf{x};\boldsymbol{\theta}^{(\text{32th iteration})}) \stackrel{(\text{A.}4)}{=} \frac{0.69679 \times 1}{0.69679 \times 1 + (1-0.69679) \times 11.09327(1-x_i)^{(11.09327 -1)}} > 0.5$.
